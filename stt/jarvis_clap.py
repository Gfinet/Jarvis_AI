...
# Kiro: focus existing instance (no -n). Set OPEN_NEW_KIRO_ON_DOUBLE_CLAP for a new window as well.
FOCUS_EXISTING_KIRO_ON_DOUBLE_CLAP = True
OPEN_NEW_KIRO_ON_DOUBLE_CLAP = True
KIRO_OPEN_FULLSCREEN = True
...
def _kiro_executable() -> str | None:
    if sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA", "")
        for sub in ("Programs\\cursor\\kiro.exe", "Programs\\kiro\\kiro.exe"):
            if local:
                p = os.path.join(local, *sub.split("\\"))
                if os.path.isfile(p):
                    return p
    return shutil.which("kiro")
...
def open_kiro_window() -> None:
    if not FOCUS_EXISTING_KIRO_ON_DOUBLE_CLAP and not OPEN_NEW_KIRO_ON_DOUBLE_CLAP:
        return
    exe = _kiro_executable()
    if not exe:
        log.warning(
            "Could not find Kiro (install app or add the `kiro` command to PATH)."
        )
        return
    popen_kw: dict = {
        "stdin": subprocess.DEVNULL,
    ...
    if sys.platform == "win32":
        popen_kw["creationflags"] = subprocess.CREATE_NO_WINDOW
    try:
        if FOCUS_EXISTING_KIRO_ON_DOUBLE_CLAP:
            focused = (
                sys.platform == "win32" and _focus_existing_kiro_window_win32()
            )
            if not focused:
                subprocess.Popen([exe], **popen_kw)
        if OPEN_NEW_KIRO_ON_DOUBLE_CLAP:
            subprocess.Popen([exe, "-n"], **popen_kw)
    except OSError as e:
        log.warning("Could not start or focus Kiro: %s", e)
        return
    if sys.platform == "win32" and KIRO_OPEN_FULLSCREEN :
        time.sleep(0.5)
        hwnd = _kiro_largest_main_hwnd_win32()
        if hwnd is not None:
            _kiro_send_f11_fullscreen_win32(hwnd)
        else:
...
def main() -> int:
    ...
    if KIRO_OPEN_FULLSCREEN and sys.platform == "win32":
    ...
    try:
        with sd.InputStream(
            ...
            while not welcome_sequence_done:
