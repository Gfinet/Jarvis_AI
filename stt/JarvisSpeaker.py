...
JARVIS_WELCOME_PHRASE = (
    ...
    "J'espère que la journée a été bonne."
)
...
def _jarvis_welcome_cache_path(text: str, model_name: str) -> Path:
    key = f"{text}|{model_name}".encode()
    digest = hashlib.sha256(key).hexdigest()[:24]
    return _jarvis_welcome_cache_dir() / f"{digest}.wav"
...
def _play_pcm_wav_file(path: Path) -> bool:
    try:
        with wave.open(str(path), "rb") as wf:
            ...
            rate = wf.getframerate()
            if ch != 1 or sw != 2:
                log.warning("Unsupported cached WAV.")
                return False
            raw = wf.readframes(wf.getnframes())
    except (OSError, wave.Error) as e:
        log.warning("Could not read cached welcome audio: %s", e)
...
class JarvisSpeaker:
    def __init__(self):
        ...
        self.model_path = base_dir / "voices" / "fr_FR-tom-medium.onnx"
        
        self.voice = None
        self.sample_rate = 22050  # Valeur par défaut de Piper

        if not self.model_path.is_file():
            log.warning(f"Modèle Piper introuvable à l'emplacement : {self.model_path}")
        else:
            try:
                log.info("Chargement du modèle Piper en local...")
                ...
                log.info("Modèle Piper chargé avec succès !")
            except Exception as e:
                log.warning(f"Erreur lors du chargement de Piper TTS: {e}")
    ...
    def speak(self, sentence: str) -> None:
        ...
        cache_path = _jarvis_welcome_cache_path(text, self.model_path.name)
        
        # 1. Si le fichier existe déjà dans le cache, on le lit directement (ultra rapide)
        if JARVIS_WELCOME_CACHE_ENABLED and cache_path.is_file():
            log.info("Playing text from cache: %s", cache_path)
            if _play_pcm_wav_file(cache_path):
                return
            log.warning("Cache miss after read failure; generating locally.")

        if not self.voice:
            log.warning("Piper voice model not loaded.")
            return

        # 2. Génération via la méthode officielle `synthesize_wav` de Piper
        try:
            cache_path.parent.mkdir(parents=True, exist_ok=True)
            tmp_path = cache_path.with_suffix(cache_path.suffix + ".tmp")
            
            # Piper écrit directement un fichier WAV propre et valide à l'emplacement indiqué
            with wave.open(str(tmp_path), "wb") as wav_file:
                self.voice.synthesize_wav(text, wav_file)
                
            tmp_path.replace(cache_path)
        except Exception as e:
            log.warning("Piper TTS generation failed: %s", e)
            if tmp_path.is_file():
                tmp_path.unlink(missing_ok=True)
            return

        # 3. Lecture immédiate du fichier fraîchement généré
